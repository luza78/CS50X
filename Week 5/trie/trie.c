...
bool check(char* word)
{
    ...
    {
        printf("Name is too long. Maximum amount of characters is %d\n", MAXCHAR);
        return false;
    }
    ...
    for (int i = 0, n = strlen(word); i < n; i++)
    {
        ...
        if (!isalpha(c))
        {
            printf("Only alphabetical characters allowed.\n");
            return false;
        }
    }

    //input is valid. copy word and make all lowercase
    int length = strlen(word);
    char *inputcopy = malloc(sizeof(char) * strlen(word) + 1);
    strcpy(inputcopy, word);
